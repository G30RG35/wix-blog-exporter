document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!statusEl || !exportBtn || !monthSelector || !loadingEl || !selectAllBtn || !debugBtn) {
  ...
  const loadAvailableMonths = async () => {
    ...
    try {
      ...
      if (!tab?.url || (!tab.url.includes('wix.com') && !tab.url.includes('wixsite.com'))) {
      ...
      if (tab.url.includes('/dashboard/') && !tab.url.includes('/blog/')) {
        throw new Error('Ve a la sección "Blog" → "Entradas" en tu dashboard de Wix para ver los posts');
      }
  ...
  const setupSelectAllButton = () => {
    ...
    selectAllBtn.addEventListener('click', () => {
      const checkboxes = monthSelector.querySelectorAll('input[type="checkbox"]');
  ...
  const setupExportButton = () => {
    exportBtn.addEventListener('click', async () => {
      const selectedMonths = Array.from(monthSelector.querySelectorAll('input[type="checkbox"]:checked'))
        .map(cb => cb.value);
      
      if (selectedMonths.length === 0) {
        statusEl.innerHTML = '<span style="color: red;">Por favor, selecciona al menos un mes para exportar</span>';
        return;
      }
      ...
      try {
        ...
        console.log('[POPUP] Iniciando exportación para meses:', selectedMonths);
        ...
        for (const monthId of selectedMonths) {
          statusEl.textContent = `Exportando ${monthId}... (${totalProcessed + 1}/${selectedMonths.length})`;
          ...
          if (response && response.success) {
            ...
            console.log(`[POPUP] Posts añadidos de ${monthId}: ${response.posts.length}`);
          } else {
            console.warn(`[POPUP] Error exportando ${monthId}:`, response?.error || 'Sin respuesta');
            statusEl.innerHTML = `<span style="color: orange;">⚠️ Error en ${monthId}: ${response?.error || 'Sin respuesta'}</span>`;
        ...
        console.log(`[POPUP] Total posts recopilados: ${allPosts.length}`);
        ...
        if (allPosts.length > 0) {
          ...
          a.href = url;
          a.download = `wix-blog-export-${new Date().toISOString().split('T')[0]}.json`;
          document.body.appendChild(a);
          a.click();
          document.body.removeChild(a);
          URL.revokeObjectURL(url);
          
          statusEl.innerHTML = `<span style="color: green;">✅ Exportados ${allPosts.length} posts correctamente</span>`;
          
          // Mostrar detalles de los posts exportados
          console.log('[POPUP] Posts exportados:', allPosts.map(p => ({ title: p.title, method: p.extractMethod })));
        } else {
          statusEl.innerHTML = '<span style="color: orange;">⚠️ No se encontraron posts para exportar. Verifica la consola para más detalles.</span>';
          console.warn('[POPUP] No se encontraron posts. Revisa la consola del content script para más información.');
        }
...
${info.isDashboard && !info.isInBlogSection ? '⚠️ Ve a Blog → Entradas en tu dashboard' : ''}
          `.trim();
          ...
          statusEl.innerHTML = '<span style="color: red;">Error obteniendo info de debug</span>';
        }
      } catch (error) {
